...
-(void)markSelected:(BOOL)blnSelected
{
    ...
    if([self.date isDateToday]){
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: [UIColor colorWithHex:0x00b0c1];
        
        self.dateLabel.textColor = (blnSelected)?[UIColor colorWithHex:0x00b0c1]:[UIColor whiteColor];
    }else{
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: [UIColor clearColor];
        
        self.dateLabel.textColor = (blnSelected)?[UIColor colorWithRed:0.0/255.0 green:176.0/255.0 blue:193.0/255.0 alpha:1.0]:[self colorByDate];
    }
...
-(UIColor *)colorByDate
{
    return [self.date isPastDate]?[UIColor colorWithHex:0x7cf1ff]:[UIColor whiteColor];
}
